...
|----|------|
| 版本 | **v2.0** |
| 状态 | **待 Founder 确认后开始 Phase 2a 编码** |
| 范围 | Phase 1（已完成）→ Phase 4（可选上云）全链路 |
| 关联 | [PRD.md](./PRD.md) · [BACKEND.md](./BACKEND.md) · [API.md](./API.md) · [AGENTS.md](./AGENTS.md) · [AGENTS-COMPARE.md](./AGENTS-COMPARE.md) |
...
**Phase 2 完成后仍缺什么（正常）：** 无真实 LLM、无自动 Dispatch、CEO Brief 仍为固定 ack——这些在 Phase 3 补。
...
- [ ] `./start.sh` → 看板视觉与 Mock 一致
- [ ] Network：`/api/v1/dashboard`，非 `mock/dashboard.json`
- [ ] `pytest backend/tests` 全绿
- [ ] 删 `data/` 重启，seed 自动重建
- [ ] OpenAPI：`http://127.0.0.1:8765/docs`
- [ ] 编排预留表存在（可空）：`agent_runs`, `handoffs`, `orchestration_events`, `deliberation_*`, `workflow_templates`
...
- [ ] 工作室正文来自 API/文件
- [ ] `data/projects/proj-acme/artifacts/` 有真实文件
- [ ] 客户 Tab 可走独立 API（或与 dashboard 重复无妨）
...
- [ ] 全路径：**HITL-3 批准 → 结项清单 → 客户 ZIP → 发周报**
- [ ] 刷新后状态保持
- [ ] 每个写接口 ≥1 集成测试
- [ ] `pulse` / `stats` / `costs.byProject` 与操作后一致
- [ ] `orchestrator_hooks` 已被所有写 API 调用（即使 Phase 2 为空实现）
...
- [ ] 手动 `Handoff(dev→ceo)` → Orchestrator 自动创建 HITL-3 inbox
- [ ] 手动 `HITLApproved` → 自动 Dispatch ops 结项 task
- [ ] `orchestration_events` 可重放整条链
- [ ] `workflow_state=deliberating` 时拒绝并行 Dispatch（为 3f 预留）
...
- [ ] Web 投递模糊 Brief → 自动创建 CEO 评估 task + inbox 条目
- [ ] 评估完成 → artifact `art-memo-*` 写入文件
- [ ] 你批 PoC/立项 → Orchestrator 自动 Dispatch 下游（接 3a 链）
- [ ] CEO Runner 使用 `role_config` 中的 api_url / key
...
- [ ] **零手动改 DB**，从 Brief 到 HITL-3 可跑通（可用 stub LLM fixture 测编排）
- [ ] 各 artifact 在工作室可见
- [ ] Handoff 触发下一角色 task 自动 `running`
- [ ] 驳回 HITL → `rework_or_escalate` 创建返工 task
...
- [ ] 每次 LLM 调用有 `agent_runs` 记录
- [ ] 经营 Tab「项目盈亏」含 Token 成本
- [ ] 超预算 → CEO Escalate 进 inbox
...
- [ ] Acme + Beta 同时跑，task/handoff 不串项目
- [ ] PoC 分支：Brief → PoC Demo → 回流立项 可演示
...
- [ ] 投递极模糊 Brief → CEO 开室（非直接 Dispatch）
- [ ] Founder 在看板旁观 turns，可不插话
- [ ] 关室后产生 Decision Memo + 正式 task 链恢复
- [ ] 会诊期间其他 Dispatch 被阻塞
